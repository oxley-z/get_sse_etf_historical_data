...
CHECKPOINT     = 'sse_checkpoint.json'   # 断点进度文件
...
def load_checkpoint():
    """读取上次保存的进度，返回 (已有results列表, 上次抓到的最早日期字符串)"""
    ...
    try:
        ...
        print(f'📂 发现断点文件，已载入 {len(results)} 个交易日，最早日期: {last_date}')
...
def save_checkpoint(results):
    """将当前进度写入断点文件（原子写入，防止写入中断导致文件损坏）"""
    last_date = min(r['date'] for r in results) if results else None
    tmp = CHECKPOINT + '.tmp'
    with open(tmp, 'w', encoding='utf-8') as f:
        json.dump({'results': results, 'last_date': last_date}, f, ensure_ascii=False)
    os.replace(tmp, CHECKPOINT)   # 原子替换，安全


def clear_checkpoint():
    if os.path.exists(CHECKPOINT):
        os.remove(CHECKPOINT)
        print(f'🗑  断点文件已清除：{CHECKPOINT}')
    ...
    try:
        ...
        while len(results) < TARGET_DAYS:
            ...
            if items:
                ...
                save_checkpoint(results)   # 每成功一天立即持久化
                print('✓')
            else:
                print('—（非交易日或无数据）')
        ...
        print(f'✅ 已保存 {len(results)} 个交易日到断点文件：{CHECKPOINT}')
        print('   下次运行脚本将自动从断点继续。\n')
        return results, False   # False 表示未完成

    return results, True   # True 表示采集完成
...
def main():
    ...
    if completed:
        ...
        print('🌐 已在浏览器中打开 HTML 看板。')
    else:
        # 中断时也生成一份当前数据的预览
        print('📊 正在生成当前数据预览（仅含已下载部分）...')
        generate_html(plot_data, OUTPUT_HTML)
        generate_excel(plot_data, OUTPUT_EXCEL)
        print('   预览文件已生成，下次续传完成后会自动覆盖更新。')
